...
# 1. Toml DumpStr - Type Checks
# def test1_exceptions_tomldumpstr_types():
#     # Tests
#     with pytest.raises(maci.error.TomlDumpStr):
#         maci.tomldumpstr(data={1,2,3})
#     with pytest.raises(maci.error.TomlDumpStr):
#         maci.tomldumpstr(data=[], indent_level='')


# # 2. Toml DumpStr - Unsupported Options or Data
# def test2_exceptions_tomldumpstr_opts_data():
#     # Tests
#     with pytest.raises(maci.error.TomlDumpStr):
#         maci.tomldumpstr(data={'k': {1,2,3}})
...
#     with pytest.raises(maci.error.TomlLoad):
#         maci.tomlload(filename='', encoding=1.0)
...
#     with pytest.raises(maci.error.TomlLoad):
#         maci.tomlload(filename=filepath, encoding='')
