...
class TestBackwardsSupport(unittest.TestCase):

    # 1. Test all prior Function attr names from 'sfcparse'
    def test1_prior_function_names(self):
        # Native
        maci.importfile
        maci.importfileraw
        maci.importattrs
        maci.exportfile
        with self.assertRaises(maci.error.GeneralError):
    ...
    def test2_prior_exception_names(self):
        # Native
