...
try:
    ...
    if DEPLOY_TYPE == 'maci-std':
        ...
        subprocess.run(('cp', '-f', f'alt_dist/{deploy_type_path}/*.py*', 'maci/')).check_returncode()
    ...
    subprocess.run(('python3', '-B', '-m', 'twine', 'upload', '--repository', DEPLOY_TYPE, 'dist/*', '--verbose')).check_returncode()
    ...
    if DEPLOY_TYPE not in ignore_github_deploy_list:
        ...
        os.chdir('maci_tag/')
        subprocess.run(f"git tag v{MACI_VERSION} -m 'Release v{MACI_VERSION}'", shell=True).check_returncode()
        subprocess.run(f"git push origin v{MACI_VERSION}", shell=True).check_returncode()
